...
```
  public/
    ...
    daily-briefing.html       # Archived daily briefing rendered from static training intel
    chat-console.html         # Analyst console now returning a static retirement notice
  ...
  assets/
    ...
    js/briefing.js            # Front-end fetcher for the archived briefing content
functions/
  api/briefing.js             # Pages Function serving curated briefing archives
```
...
2. Create a `.dev.vars` file in the project root with any secrets required for optional integrations:
   ...
   EOF
   ```
   The daily briefing and chat console now use archived content and no longer require Cloudflare Workers AI credentials.
3. Run the Pages preview with Functions support:
   ```bash
   npx wrangler pages dev public
   ```
4. Open the printed URL in your browser to view the site. The `/api/briefing` route now serves the curated archive used by the front end.
...
4. Trigger a deploy. Cloudflare will publish every file inside `public` and execute `functions/api/briefing.js` for `/api/briefing` requests to serve the archived content.
...
- **Daily Briefing**: The front end calls `/api/briefing`, which now returns a curated archive of training intel. Update the content in `functions/api/briefing.js` if you want to rotate or expand the static entries.
